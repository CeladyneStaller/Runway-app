                      ...
                      <tr className="emprow-sub"><td colSpan={7}>
                        <div className="review">
                          ...
                          <span className="rvbody">
                            {v.fires === null
                              ? <>Repayment is a {Math.round((r.royaltyPct || 0) * 100)}% royalty on {r.royaltyBase || "profit"}{v.trig > 0 && <> once cumulative revenue passes <b className="num">{moneyFull(v.trig)}</b></>}, running until <b className="num">{moneyFull(v.cap)}</b> — {r.capMultiple || 5}× what you took — has been paid. {/* NAME WHAT THE NUMBER IS. "this projection reaches $3.4m" left the reader guessing at what reached it — cumulative REVENUE, over the whole horizon, on the most optimistic toggles. */}Cumulative revenue reaches <b className="num">{moneyFull(v.cum)}</b> over {v.months || 36} months even at its most optimistic, short of the <b className="num">{moneyFull(v.trig)}</b> threshold — so the royalty never starts inside this model. <b>The obligation is real and it is not in this picture.</b></>
