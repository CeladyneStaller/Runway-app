...
// SUBSCRIPTIONS ARE EDITED UNDER SALES, not cash flow. Recurring revenue from customers is something
// you sell; cash flow is where the consequence shows up.
const openSubs = (initial) => {
  function H() { const [d, setD] = useState(initial); return <RunwayApp doc={d} setDoc={setD} />; }
  const { container } = render(<H />);
  fireEvent.click([...container.querySelectorAll("button.nav")].find(b => /^Sales/.test(b.textContent)));
  fireEvent.click([...container.querySelectorAll("button.subtab")].find(b => /^Subscriptions/.test(b.textContent)));
  return container;
};
...
describe("the subscription panel", () => {
  ...
  it("shows the steady state the churn implies", () => {
    ...
    const c = openSubs(withCash({ saas: [{ ...blankSaas(), arpu: 100, newPerMonth: 20, churnPct: 10 }] }));
  ...
  it("warns when no churn was entered, because that is unbounded growth", () => {
    const c = openSubs(withCash({ saas: [{ ...blankSaas(), arpu: 100, newPerMonth: 20, churnPct: 0 }] }));
    expect(c.textContent).toMatch(/grows without limit/i);
  });

  it("warns when nothing new is coming in, because that is decay", () => {
    const c = openSubs(withCash({ saas: [{ ...blankSaas(), startCustomers: 80, arpu: 100, churnPct: 5 }] }));
  ...
  it("counts subscription MRR in the recurring-revenue figure", () => {
    ...
    // expands to per-month one-time lines. Read on the CASH-FLOW tab; entered under Sales.
    const c = openRevenue(withCash({ saas: [{ ...blankSaas(), startCustomers: 100, arpu: 250 }] }));
  ...
  it("deletes cleanly", () => {
    const c = openSubs(withCash({ saas: [{ ...blankSaas(), arpu: 100 }] }));
    expect(c.querySelector(".saas-card")).toBeTruthy();
...
describe("recording MRR against a book", () => {
  ...
  it("shows each record against what was projected, and the gap", () => {
    const c = openSubs(withRec({ 0: 8000 }));
  ...
  it("says 'on plan' rather than a zero when they agree", () => {
    const c = openSubs(withRec({ 0: 10000 }));
  ...
  it("offers re-basing only when the record disagrees with the model", () => {
    ...
    expect(btn(openSubs(withRec({ 0: 8000 })), /Re-base forecast/)).toBeTruthy();
    expect(btn(openSubs(withRec({ 0: 10000 })), /Re-base forecast/)).toBeFalsy();
  });
...
describe("month labels are real dates", () => {
  it("names the actual month, not a transposed one", () => {
    ...
    const c = openSubs(doc);
    expect(c.querySelector(".saas-rec-m").textContent).toBe("Oct 26");
    expect(c.textContent).toMatch(/Starts Oct 26/);
  });
});

describe("the funnel from Sales to Revenue", () => {
  const withSubs = () => withCash({ saas: [{ ...blankSaas(), startCustomers: 100, arpu: 250 }] });

  it("MRR counts toward recurring revenue on the cash-flow tab", () => {
    // Moving the INPUT must not move the money. The consequence still lands where consequences land.
    const c = openRevenue(withSubs());
    expect(c.textContent).toMatch(/incl\. subscriptions/);
  });

  it("and the cash-flow tab says where that money was entered", () => {
    // Otherwise there is an unexplained gap between the stat at the top and the lines listed below it.
    const c = openRevenue(withSubs());
    expect(c.textContent).toMatch(/\$25,000\/mo/);
    expect(c.textContent).toMatch(/entered under/i);
    expect(c.textContent).toMatch(/Sales/);
  });

  it("with a way straight there", () => {
    function H() { const [d, setD] = useState(withSubs()); return <RunwayApp doc={d} setDoc={setD} />; }
    const { container } = render(<H />);
    fireEvent.click([...container.querySelectorAll("button.nav")].find(b => /Cash flow/.test(b.textContent)));
    fireEvent.click([...container.querySelectorAll("button.subtab")].find(b => /^Revenue$/.test(b.textContent)));
    fireEvent.click([...container.querySelectorAll("button")].find(b => /Sales . Subscriptions/.test(b.textContent)));
    expect(container.querySelector(".saas-card")).toBeTruthy();
  });

  it("says nothing about subscriptions when there are none", () => {
    const c = openRevenue(withCash());
    expect(c.textContent).not.toMatch(/entered under/i);
    expect(c.textContent).not.toMatch(/incl\. subscriptions/);
  });

  it("shows MRR in the Sales summary, beside booked and pipeline", () => {
    function H() { const [d, setD] = useState(withSubs()); return <RunwayApp doc={d} setDoc={setD} />; }
    const { container } = render(<H />);
    fireEvent.click([...container.querySelectorAll("button.nav")].find(b => /^Sales/.test(b.textContent)));
    expect(container.textContent).toMatch(/MRR this month/);
  });
});
